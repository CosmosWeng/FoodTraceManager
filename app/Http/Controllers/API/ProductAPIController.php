...
class ProductAPIController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $products  = $this->productRepository->with(['category:id,name'])
                        ->paginate($request->get('limit', null))
                        ->toArray();
        if ($products['total'] == 0) {
            return $this->sendError('Products is empty');
        }

        return $this->sendPaginateResponse($products, 'Products retrieved successfully');
    }
    ...
    {
        /** @var Product $product */
        $product = $this->productRepository->with(['category:id,name'])->findWithoutFail($id);
